...
static void
bgp_vrf_process_one (struct bgp_vrf *vrf, afi_t afi, struct bgp_node *rn,
                     struct bgp_info *old_select, struct bgp_info *new_select)
{
  ...
  if (sel && new)
    {
      int eq = 0;
      if (!bgp_info_cmp (vrf->bgp, new, sel, &eq))
        return;
  ...
  for (iter = vrf_rn->info; iter; iter = iter->next)
    {
      int eq = 0;
      if (newsel == NULL || bgp_info_cmp (vrf->bgp, iter, newsel, &eq))
        newsel = iter;
    }
  if (sel == newsel)
    return;
    ...
    for (i = 0; i < (size_t)old_ecom->size; i++)
      {
        ...
        for (ALL_LIST_ELEMENTS_RO(rt_sub->vrfs, node, vrf))
          bgp_vrf_process_one (vrf, afi, rn, old_select, withdraw ? NULL : new_select);
      }
    ...
    for (i = 0; i < (size_t)new_ecom->size; i++)
      {
        ...
        if (!found)
          for (ALL_LIST_ELEMENTS_RO(rt_sub->vrfs, node, vrf))
            bgp_vrf_process_one (vrf, afi, rn, NULL, new_select);
      }
}
